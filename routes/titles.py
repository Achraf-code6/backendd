...
async def generate_content(data: GenerateRequest):
    ...
    if any(k in data.title.lower() for k in noise_keywords) and (not data.description or len(data.description) < 20):
        return GenerateResponse(
            ...
            new_description="Le scraper n'a récupéré que du texte d'interface. Cela arrive si un popup (ex: 'Entrer une adresse') bloque la vue ou si vous n'êtes pas sur une page listing produit. Veuillez fermer les popups et réessayer."
        )
